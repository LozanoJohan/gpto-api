...
THINK_STEPS = "Analyse and think step by step what do you have to do in order to answer user's question. Be specific indentifying which data is the user trying to know and the constraints is giving. Decide which tables and fields are relevant from the folowing schemas: {database_schema} DO NOT GIVE ANY SQL CODE. Don't give examples, avoid titles and bullet points. Answer must have 1 paragraph in length".format(database_schema=database_schema)
